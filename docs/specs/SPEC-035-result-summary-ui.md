...
### 4.2 残す・強化する要素
- **共通パラメーターバー**：HUD 直下に固定表示し、S3/S4/S10/S9 でも現在の5素養を常に見せる
...
### 5.1 方針
S3/S4/S10/S9 では、画面内カードとしての「獲得した素養」を原則表示しない。代わりに `#param-strip` を HUD 直下の同じ位置に固定し、常に現在値を見せる。これにより、プレイヤーはどの画面でも同じ場所でパラメーターを確認できる。

### 5.2 旧結果カードの扱い
`#result-soyou-card`, `#day-summary-soyou-card`, `#highlight-soyou-card` は DOM 互換のため残すが hidden とする。差分演出は将来 `#param-strip` 側へ移す。

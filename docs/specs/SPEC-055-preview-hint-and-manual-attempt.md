...
- 「あと一歩」の判定（v2 修正版）：
  - `enterLocation` トリガなら：**その場所が「今朝の親遣い先」になっている** ときのみ near。
    - 解禁済みリストだけで判定すると毎日鳴ってしまいサプライズ性が消えるため、より厳格な「今日そこに行く」だけに絞る
...
| ミッション | manualAttempt 化 | previewHint 内容 | minAge |
|---|---|---|---|
| `m_picturebook_first`（**新規・1 年目体験用**） | ✅ playCountAtLeast | 「お母さんが何か気になってるみたい?」 | 1 |
| `m_puzzle_9` | ✅ | 「児童館で誰かが見守っている?」 | 2（児童館解禁が age=2〜） |
| `m_share_toy` | ✅ | 「公園で誰かが寂しそう?」 | 1（v2 で 2→1 へ引き下げ） |
| `m_song_10` | ✅（playCountAtLeast 型） | 「お父さん・お母さんが何か聞きたそう?」 | 1（v2 で 2→1、必要数 10→8） |

## 9. 1 年目（age=1）でも本機能が体験できる保証

> ユーザーフィードバック (PR #25): 「1年目（チュートリアル）でも上記機能が体験できるようにバランス調整をお願いします」

### 9.1 課題
初版では既存ミッション 3 件すべてが `minAge: 2` だったため、player.age が初期値 1 のまま 119 日（最初の誕生日まで）は **予告ヒントも manualAttempt も体験できない** 問題があった。

### 9.2 対応
- **新規ミッション `m_picturebook_first` を追加**：picturebook は phase0（Day 1〜7）に解禁される最初の遊びなので、Day 1 から累計が積み上がる。
  - 発端：picturebook 累計 3 回（70% = 2 回時点で予告ヒント表示）
  - 達成：picturebook 累計 6 回 + intellect ≥ 10
  - **Day 8〜10 頃に予告ヒント、Day 12〜14 頃にプロンプト体験** が可能
- **`m_song_10` の minAge を 2→1、累計 10→8 へ引き下げ**：song は Day 4 以降の phase0 後半で解禁。1 年目でも到達可能なバランスに。
- **`m_share_toy` の minAge を 2→1、必要素養を 30→15 に引き下げ**：phase1（Day 8〜）以降に near_park 親遣いが発生するため体験可能。

### 9.3 体験タイムライン（1 年目）
| Day 範囲 | 体験できるミッション要素 |
|---|---|
| 1〜4 | `m_picturebook_first` 発端（picturebook 累計 3） |
| 5〜7 | 予告ヒント表示（70% 達成 = picturebook 累計 4） |
| 8〜14 | manualAttempt 達成プロンプト（picturebook 累計 6 + intellect ≥ 10）、または `m_song_10` の予告/プロンプト |
| 15〜 | phase2 解禁 + age=1 残期間で `m_share_toy`（near_park 親遣い時） |

## 10. 改訂履歴
- 2026-04-25: 初版（SPEC-053 §3.1 §3.2 への対応）
- 2026-04-25 v2: 1 年目体験保証（`m_picturebook_first` 追加、`m_song_10` / `m_share_toy` の minAge を 1 に引き下げ）

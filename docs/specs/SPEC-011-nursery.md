...
| 関連仕様 | SPEC-001, SPEC-002, SPEC-007, SPEC-010, SPEC-019, SPEC-020, SPEC-021, SPEC-047（場所システム）, SPEC-056（イベントモーダル）, SPEC-057（感染症と治癒） |
| ステータス | Active（実装対象） |
| 最終更新 | 2026-04-26 |
...
> **v3 改修（2026-04-26、@spec SPEC-057）**：
>
> 1. **S2/コアタイムをスキップ**：`_specialDayMode === "infection"` のとき `goChooseFromToday()` で即 `goSleep()`。プレイヤーは朝のモーダル → S10 連絡帳サマリの 1 タップフローで 1 日を終える
> 2. **毎晩治癒判定**：3 日固定ではなく、残日数別の自然治癒（残 3 日 = 0%、残 2 日 = 30%、残 1 日 = 50%）で前倒しで治る可能性
> 3. **お医者さん通院**：感染症中の親遣い抽選は **clinic 90% / home 10%** に絞る。clinic 訪問日は `_visitedClinicToday = true`、その夜の `maybeHealInfection()` で **100% 治癒**
> 4. **健康な日に病院は出ない**：`clinic.parentalOutingWeekdayWeight = 0` で通常プールから除外
